...
from PyQt5.QtWidgets import QApplication, QMainWindow, QMessageBox, QLabel, QVBoxLayout, QHBoxLayout, QPushButton
import sys
import sqlite3
import requests
...
class Logon_Logic(QMainWindow, Logon_Interface):
    def __init__(self):
        ...
        self.setupUi(self)
        self.id = (1, )
        if os.path.exists("DB_for_project.db"):
            self.con = sqlite3.connect('DB_for_project.db')
            self.cur = self.con.cursor()
        else:
            self.con = sqlite3.connect('DB_for_project.db')
            self.cur = self.con.cursor()
            self.cur.execute("""CREATE TABLE mainTable (
    id        INTEGER NOT NULL
                      UNIQUE
                      PRIMARY KEY,
    login     STRING  UNIQUE
    ...
    def check_add_logon(self):
        def addSubjectDB(s):
            ...
            for i in range(len(timeDB)):
                if i % 5 == 1:
                    subjects.append(timeDB[i][:])
                    homeworks.append(" ".join(timeDB[i + 1][:].split()))
            self.cur.execute(f"UPDATE mainTable SET subjects = ? WHERE login = ?",
                             ('||'.join(subjects), datas['main_login2']))
            self.cur.execute(f"UPDATE mainTable SET homeworks = ? WHERE login = ?",
                             ('||'.join(homeworks), datas['main_login2']))
        ...
        elif "Мой дневник" not in requests.post(link, data=datas, headers=header).text:
            QMessageBox.about(self, "Аккаунт не школьника", "Этот аккаунт явно не школьника.\nВведите другие данные")
        else:
            s = requests.Session()
    ...
    def mainMenu(self):
        global MainMenu_Logic, MainMenu_Logic_1, Grade_Logic, Grade_Logic_1, Homework_Logic, Homework_Logic_1
...
class Homework_Interface(object):
    def setupUi(self, MainWindow):
        ...
        self.frame.setGeometry(QtCore.QRect(30, 50, 361, 541))
...
class Homework_Logic(QMainWindow, Homework_Interface):
    def __init__(self):
        ...
        self.setupUi(self)
        self.cur = sqlite3.connect('DB_for_project.db').cursor()
        self.subjects = self.cur.execute("SELECT subjects FROM mainTable WHERE id = ?", (Logon_Logic_1.id[0],)).fetchone()[0]
        self.homeworks = self.cur.execute("SELECT homeworks FROM mainTable WHERE id = ?", (Logon_Logic_1.id[0],)).fetchone()[0]
        for i in self.subjects.split("||"):
            self.frame2 = QtWidgets.QFrame(self.centralwidget)
            self.Vlayout = QVBoxLayout()
            self.Hlayout = QHBoxLayout(self.frame2)
            self.subject_lbl = QLabel(i)
            self.subject_lbl.setFont(QtGui.QFont("MS Shell Dlg 2", 15))
            self.subject_lbl.setMinimumSize(300, 20)
            self.homework_lbl = QLabel(".\n".join(self.homeworks.split("||")[self.subjects.split("||").index(i)].split(".")))
            self.homework_lbl.setFont(QtGui.QFont("MS Shell Dlg 2", 10))
            self.homework_lbl.setMinimumSize(300, 40)
            self.btn = QPushButton("x")
            self.btn.setFont(QtGui.QFont("MS Shell Dlg 2", 35))
            self.btn.setMinimumSize(60, 60)
            self.btn.setStyleSheet("background-color: rgb(240, 170, 100);")
            self.btn.clicked.connect(self.removeHomework)
            self.Vlayout.addWidget(self.subject_lbl)
            self.Vlayout.addWidget(self.homework_lbl)
            self.Hlayout.addLayout(self.Vlayout)
            self.Hlayout.addWidget(self.btn)
            self.verticalLayout.addWidget(self.frame2)
        self.back_btn.clicked.connect(self.back)

    def removeHomework(self):
...
if __name__ == "__main__":
    ...
    Planner_Logic_1 = ""
    Logon_Logic_1.show()
    sys.excepthook = lambda cls, exception, traceback: sys.excepthook(cls, exception, traceback)
    sys.exit(app.exec_())
